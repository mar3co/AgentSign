...
- send — POST /v1/documents. PDF bytes (base64), not a public pdf_url. The MCP tool sends PDFs only; DOCX uploads are REST-only (multipart, converted to PDF). Optional Bearer sign_live_ key. Without a key, send starts an OTP one-off; check sender email. sign_tmp_ cannot send or list. Signer objects may include kind (human|agent) and agent slug.
