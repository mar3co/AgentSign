...
**Packaging:** two columns. **Free** (login optional): 7 days to sign, **7-day shredder** after keep window, live keys after login, quiet ~20 sends / 30 days, “Sent with AgentSign” on the appearance page. **Pro $19/mo** (Stripe Checkout): 1-year keep, cap lift, footer off, plus branding, saved packets, team invites (soft cap 10, not billed per person), and **10 named agents**. No seats, no per-document, no Enterprise page. Self-host is forever: set `SELF_HOST=1` to entitle Pro extras without a Stripe plan.

**Surface:** REST (`/v1/envelopes`, `/v1/verify`; Pro/self-host `/v1/branding`, `/v1/packets`, `/v1/team`, `/v1/agents`) + **OpenAPI** (`/openapi.json`) + **llms.txt** + MCP (stdio and HTTP): `send`, `status`, `download`, `attest`, `reject`, `verify`, `list_packets`, `send_packet`. No `sign` tool. Humans Finish. Agents Attest. Ceremony logo is `GET /s/:token/logo` (signing token, not a public account URL).
